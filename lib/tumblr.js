      ...
      if (key.match(/x-ratelimit/)) {
        ...
        if (key.match(/remaining$/)) {
          if (ratelimit[key] <= 5) {
            ...
            process.dm.bugsnag.notify("TUMBLR RATELIMIT HEADER WARRNING", ratelimit);
            //return callback()
          }
        }
      }
...
*/
 if (response.statusCode >= 400) {
      ...
      return callback(new Error('API error: ' + response.statusCode + ' ' + err), body, ratelimit);
    }
